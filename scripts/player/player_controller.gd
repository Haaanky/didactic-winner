...
## Main player controller. Handles 8-directional movement, stamina, and interaction.
## All needs and inventory are delegated to child components.
...
@export var interact_ray: RayCast2D
...
func _calculate_speed() -> float:
	...
	var encumbrance_penalty: float = encumbrance_ratio * ENCUMBRANCE_SPEED_PENALTY
	var base: float = RUN_SPEED if _is_running else MOVE_SPEED
	return base * (1.0 - encumbrance_penalty)


func _try_interact() -> void:
	if interact_ray == null:
		return
	interact_ray.target_position = _last_direction.normalized() * 32.0
	interact_ray.force_raycast_update()
	if interact_ray.is_colliding():
		var target: Object = interact_ray.get_collider()
		if target is Node:
			interacted_with.emit(target as Node)
			EventBus.interacted_with.emit(target as Node)
